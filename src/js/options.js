...
const options = [
  ...
  },
  {
    ...
    tip: '<p>Добавляет кнопку в двух местах: в панели управления каждой карточки на канбан-доске — по клику сразу постит комментарий вида «TAGALL, ...» без открытия задачи; и в тулбаре редактора комментария на странице задачи — по клику просто вставляет тот же текст в поле ввода, без отправки.</p><p>Текст после TAGALL можно изменить в настройках.</p>',
    author: 'Владислав Т.',
    new: true,
    groups: ['tasks'],
    popularity: 50,
    action: async ({sessionId, options}) => {
      ...
      tagallButton(sessionId, options.tagallButtonSuffix);
    },
    options: [
      {
  ...
  },
  {
    ...
    options: [
      ...
      },
      {
        ...
        name: 'Без звука',
        tip: 'Показывает системное уведомление без стандартного звука браузера/ОС.',
        default: true,
      },
    ],
  },
  ...
  },
  {
    ...
    tip: '<p>Рядом с таймером рабочего времени показывает время окончания рабочего дня по формуле: текущее время + (длительность дня − отработано). Клик по времени копирует его.</p>',
    author: 'Константин Ф.',
    new: true,
    groups: ['tasks'],
    popularity: 55,
    action: async ({options}) => {
      ...
      worktimeEnd(options.worktimeEndDayHours);
    },
    options: [
      {
