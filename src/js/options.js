...
export default [
  ...
  {
    key: 'osNotifications',
    name: 'Системные уведомления (OS)',
    tip: '<p>Показывает всплывающие уведомления Bitrix как уведомления операционной системы — виден текст сразу по звуку, даже если вкладка с Bitrix не активна.</p><p>Клик по уведомлению открывает или фокусирует нужную задачу.</p>',
    new: true,
    groups: ['notifications'],
    popularity: 50,
    action: async ({options}) => {
      const {osNotifications} = await import('/src/js/actions/os-notifications');
      osNotifications(options);
    },
    options: [
      {
        key: 'osNotificationsOwnOnly',
  ...
  },
  {
    ...
    popularity: 50,
    action: () => {
      import('/src/js/actions/redirect-after-task-create');
    },
  },
  {
    key: 'autoTaskTitle',
    name: 'Название задачи по умолчанию',
    tip: '<p>При создании подзадачи или задачи из комментария поле «Название» автоматически подставляется названием исходной задачи, если ещё не заполнено.</p>',
    new: true,
    popularity: 50,
    action: async ({sessionId}) => {
  ...
  },
  {
    ...
    tip: '<p>Показывает рядом с таймером «Начать/Закончить работу» в шапке расчётное время окончания дня: время начала + перерыв (не меньше настроенного минимума) + длительность рабочего дня.</p><p>Считается один раз при открытии попапа и при каждом переходе (пауза/продолжение/завершение) — не тикает вместе с таймером каждую секунду. Если сейчас идёт пауза, рядом показывается предупреждающая иконка — пока перерыв длится дольше минимума, реальное время окончания сдвигается дальше, а расчёт остаётся прежним до перезагрузки страницы.</p>',
    new: true,
    groups: ['tasks'],
    popularity: 55,
    action: async ({options}) => {
      const {worktimeEnd} = await import('/src/js/actions/worktime-end');
      worktimeEnd(options.worktimeEndDayHours, options.worktimeEndMinBreakHours);
    },
    options: [
      {
      ...
      {
        key: 'worktimeEndMinBreakHours',
        name: 'Минимальный перерыв (часы)',
        tip: 'Пока фактический перерыв короче этого значения, в расчёте используется оно; если перерыв длится дольше — используется фактическое время.',
        default: 1,
        type: optionTypes.NUMBER,
      },
    ],
