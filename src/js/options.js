...
const options = [
  ...
  {
    key: 'taskDynamics',
    name: 'Динамика задач группы (виджет)',
    tip: '<p>Добавляет кнопку «Динамика» в канбане. Показывает, как менялся процесс группы за длинный период: входящий поток задач против пропускной способности, сколько задача ждёт от постановки до закрытия, что залежалось в планах, рост выработки и состава команды, доля хотфиксов, декомпозиция.</p><p>Считает по всем задачам канбана, а не по выбранным участникам: это взгляд на процесс, а не отчёт по людям. Есть сравнение с предыдущим периодом, события (даты изменений в процессе) со сводкой «до и после», выгрузка и AI-анализ.</p><p>Внутри виджета есть настройки: колонки с планами и архивные колонки, пороги, события.</p>',
    author: 'Данил М.',
    new: true,
    beta: true,
    groups: ['analytics'],
  ...
  },
  {
    ...
    groups: ['tasks'],
    popularity: 70,
    action: () => import('/src/js/actions/commit-button'),
  },
  {
    ...
    groups: ['tasks'],
    popularity: 70,
    action: () => import('/src/js/actions/kanban-commit-button'),
  },
  {
    key: 'tagallButton',
    name: 'Кнопка «TAGALL, на проде»',
    tip: '<p>Добавляет кнопку в тулбар редактора комментария на странице задачи — по клику вставляет комментарий вида «TAGALL, ...» в поле ввода, без отправки.</p><p>Отдельной галкой такую же кнопку можно добавить на карточки канбан-доски — там она сразу публикует комментарий, без открытия задачи.</p><p>Текст после TAGALL можно изменить в настройках.</p>',
    author: 'Владислав Т.',
    new: true,
    groups: ['tasks'],
    popularity: 50,
    action: async ({sessionId, options}) => {
      const {tagallButton} = await import('/src/js/actions/tagall-button');
      tagallButton(sessionId, options);
    },
    options: [
      {
        key: 'tagallButtonSuffix',
        name: 'Текст после TAGALL',
        tip: 'Начальные запятая и пробел не нужны — подставляются автоматически. Если оставить пустым, используется «на проде».',
        type: optionTypes.TEXT,
        placeholder: 'на проде',
      },
      {
        key: 'tagallButtonKanban',
        name: 'Кнопка на карточках канбана',
        tip: 'Добавляет кнопку в панель управления каждой карточки на канбан-доске — по клику сразу публикует комментарий, без открытия задачи. Кнопка в тулбаре редактора комментария на странице задачи добавляется всегда.',
        default: false,
      },
      {
        key: 'tagallButtonAuthorOnly',
        name: 'Имя постановщика + на проде',
        tip: 'Вместо TAGALL (все участники задачи) тегается только постановщик задачи — по его ID.',
      },
    ],
  },
  {
  ...
  {
    key: 'exportGroupTasks',
    name: 'Экспорт задач группы (виджет)',
    tip: '<p>Добавляет кнопку «Экспорт» в канбане. Позволяет выгрузить список задач группы — с фильтрами по дате создания, статусу и колонке канбана.</p><p>Можно выбрать набор колонок, формат выгрузки (CSV или JSON) и разделитель CSV, а перед выгрузкой посмотреть результат в таблице предпросмотра.</p>',
    author: 'Данил М.',
    groups: ['tasks'],
    popularity: 15,
    new: true,
  ...
  },
  {
    ...
    groups: ['tasks'],
    popularity: 70,
    action: () => import('/src/js/actions/fix-links'),
  },
  {
    key: 'showComments',
  ...
  },
  {
    ...
    options: [
      ...
      {
        key: 'notificationDetailsFilter',
        name: 'Фильтр уведомлений',
        tip: 'Показывает панель фильтров над списком уведомлений.',
        default: true,
      },
    ],
  },
  ...
  {
    key: 'alertPage',
    name: 'Улучшения страницы всех уведомлений',
    tip: '<p>Набор доработок страницы /alert/ — полного списка уведомлений: кнопка открытия всех задач сразу, название группы у блока задачи и фильтр сообщений по группам. Каждая доработка включается своим чекбоксом ниже.</p><p>Счётчик уведомлений вверху страницы при этом всегда остаётся актуальным — Bitrix не пересчитывает его после удаления сообщений.</p>',
    author: 'Алексей К.',
    new: true,
    groups: ['notifications'],
    popularity: 15,
    action: async ({options}) => {
      const {alertPage} = await import('/src/js/actions/alert-page');
      alertPage(options);
    },
    options: [
      {
        key: 'alertPageOpenAllTasks',
        name: 'Кнопка «Открыть все задачи»',
        tip: 'Добавляет рядом с кнопкой удаления кнопку, которая открывает в новых вкладках все задачи из списка уведомлений — каждую по одному разу. Если задач больше десяти, сначала спросит подтверждение.',
        default: true,
      },
      {
        key: 'alertPageGroupName',
        name: 'Название группы в заголовке задачи',
        tip: 'Дописывает в заголовок блока уведомлений название группы, к которой относится задача, — ссылкой на список задач этой группы.',
        default: true,
      },
      {
        key: 'alertPageGroupFilter',
        name: 'Фильтр уведомлений по группам',
        tip: 'Добавляет к фильтрам страницы список групп с количеством уведомлений в каждой. Выбор группы оставляет на странице только её уведомления.',
        default: true,
      },
    ],
  },
  {
    ...
    options: [
      ...
      {
        key: 'removeSystemNotificationsClosedTasks',
        name: 'Уже закрытые задачи (!)',
        tip: 'Удаляет уведомления по задачам, которые уже закрыты — но только если уведомление появилось до момента закрытия задачи. Уведомления, пришедшие позже закрытия, не трогает.',
      },
      {
      ...
      {
        key: 'removeSystemNotificationsStatus',
        name: 'Статусные уведомления',
        tip: 'Удаляет tagall-уведомления о статусе или готовности («готово», «в проде», «задеплоил» и т. п.).',
      },
    ],
  },
  {
    ...
    author: 'Алексей К.',
    popularity: 10,
    action: () => import('/src/js/actions/auto-choice-user'),
  },
  {
    ...
    groups: ['tasks'],
    popularity: 80,
    action: () => import('/src/js/actions/invisible-mentions'),
  },
  {
    key: 'groupTitle',
    ...
    author: 'Алексей К.',
    popularity: 10,
    action: () => import('/src/js/actions/group-title'),
  },
  {
  ...
  },
  {
    ...
    tip: '<p>Показывает всплывающие уведомления Bitrix как браузерные (системные) уведомления — виден текст сразу по звуку, даже если вкладка с Bitrix не активна.</p><p>Клик по уведомлению открывает или фокусирует нужную задачу.</p>',
    author: 'Константин Ф.',
    new: true,
    groups: ['notifications'],
    popularity: 50,
    action: async ({options}) => {
      const {browserNotifications} = await import('/src/js/actions/browser-notifications');
      browserNotifications(options);
    },
    options: [
      {
        key: 'browserNotificationsOwnOnly',
        name: 'Не показывать чужие уведомления',
        tip: 'Показывает системное уведомление, только если в тексте есть ваше имя или TAGALL — как в «Автозакрытие чужих уведомлений». Требует указанные имя и фамилию в профиле, иначе фильтр не применяется.',
        default: true,
      },
      {
        key: 'browserNotificationsSilent',
        name: 'Без звука',
        tip: 'Показывает системное уведомление без стандартного звука браузера/ОС. Звук может всё равно звучать в зависимости от настроек уведомлений вашей операционной системы.',
        default: true,
      },
    ],
  },
  ...
  },
  {
    ...
    groups: ['tasks'],
    popularity: 80,
    action: () => import('/src/js/actions/status-markers'),
  },
  {
    key: 'openInNewTab',
  ...
  },
  {
    ...
    author: 'Константин Ф.',
    popularity: 50,
    action: () => import('/src/js/actions/auto-auditor'),
  },
  {
    key: 'bitrixBugfixes',
    name: 'Исправление багов Bitrix',
    tip: '<p>Набор точечных исправлений известных багов Bitrix24 Plan. Каждое исправление включается своим чекбоксом ниже.</p>',
    new: true,
    popularity: 50,
    action: async ({options}) => {
      const {bitrixBugfixes} = await import('/src/js/actions/bitrix-bugfixes');
      bitrixBugfixes(options);
    },
    options: [
      {
        key: 'bitrixBugfixesRedirectAfterTaskCreate',
        name: 'Редирект на просмотр после создания задачи',
        tip: '<p>После сохранения новой задачи через нативную форму создания (открытой не во всплывающем окне, а отдельной вкладкой) Bitrix оставляет вас на странице редактирования вместо просмотра — эта опция сразу перенаправляет на страницу просмотра созданной задачи.</p>',
        author: 'Артём А.',
        default: true,
      },
      {
        key: 'bitrixBugfixesScrollUserLists',
        name: 'Скролл у списков наблюдателей и соисполнителей',
        tip: '<p>Если в задаче много наблюдателей или соисполнителей, их список ограничивается высотой 500px и получает собственную прокрутку вместо растягивания всей боковой панели задачи.</p>',
        author: 'Данил М.',
        default: true,
      },
      {
        key: 'bitrixBugfixesTimemanPopupPosition',
        name: 'Положение окна «Редактирование рабочего дня»',
        tip: '<p>Окно редактирования рабочего дня открывается за краем экрана и обрезается. Опция сразу после открытия заставляет Bitrix пересчитать его положение — так же, как это делает изменение размера окна браузера вручную.</p>',
        author: 'Данил М.',
        default: true,
      },
    ],
  ...
  {
    key: 'autoTaskTitle',
    name: 'Название задачи по умолчанию',
    tip: '<p>При создании подзадачи или задачи из комментария поле «Название» заполняется названием исходной задачи.</p><p>Текст комментария, который Bitrix подставляет в это поле сам, заменяется — а вот если вы успели начать вводить название вручную, останется ваш вариант.</p>',
    author: 'Артём А.',
    new: true,
    popularity: 50,
    action: async ({sessionId}) => {
      const {autoTaskTitle} = await import('/src/js/actions/auto-task-title');
      autoTaskTitle(sessionId);
    },
  },
  {
    key: 'worktimeEnd',
    name: 'Расчётное время окончания рабочего дня',
    tip: '<p>Рядом с таймером рабочего времени показывает время окончания рабочего дня по формуле: текущее время + (длительность дня − отработано) + остаток обеда. Клик по времени копирует его.</p>',
    author: 'Константин Ф.',
    new: true,
    groups: ['tasks'],
    popularity: 55,
    action: async ({options}) => {
      const {worktimeEnd} = await import('/src/js/actions/worktime-end');
      worktimeEnd(options.worktimeEndDayHours, options.worktimeEndLunchHours);
    },
    options: [
      {
        key: 'worktimeEndDayHours',
        name: 'Длительность рабочего дня (часы)',
        default: 8,
        type: optionTypes.NUMBER,
        width: '90px',
      },
      {
        key: 'worktimeEndLunchHours',
        name: 'Обед (часы)',
        tip: 'Обед не входит в отработанное время, поэтому сдвигает время окончания. Если перерыв в Bitrix уже длиннее обеда, учитывается фактический перерыв. 0 — не учитывать обед.',
        default: 1,
        type: optionTypes.NUMBER,
        width: '90px',
      },
    ...
    options: [
      {
        ...
        choices: [
          ...
          {label: 'AI Cats (нужен VPN)', value: 'aicats'},
        ],
      },
    ],
  },
  {
    ...
    author: 'Данил М.',
    popularity: 0,
    action: () => import('/src/js/actions/screen-blur'),
  },
  {
    ...
    new: true,
    beta: true,
    settingsUrl: 'whats-new.html?calls=1',
    action: async () => {
...
/**
 * Собирает карту «ключ опции → значение по умолчанию» по всему реестру, включая вложенные опции.
 * Служит и начальным состоянием формы настроек (попап, «Что нового»), и списком известных ключей
 * при импорте настроек. Живёт здесь, а не в компонентах: реестр опций — единственный владелец
 * этой схемы, а три отдельные копии функции уже успели разойтись между собой.
 * @returns {Record<string, string|number|boolean|Array|null>} Значения по умолчанию по ключам опций.
 */
export function getDefaultOptions() {
  const result = {};

  function collect(optionsList) {
    optionsList.forEach((option) => {
      ...
      if (option.options) collect(option.options);
    });
  }
