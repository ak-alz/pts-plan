...
async function copyLink() {
  ...
  try {
    ...
    // Буфер обмена может быть недоступен (нет разрешения, документ не в фокусе). Иконка тогда
    // не сменится на галочку — ссылка рядом видна целиком, её можно скопировать вручную
    console.warn(error);
    return;
  }
