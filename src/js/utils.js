...
export function stringToPastelColor(str) {
  ...
  // 2. Преобразуем хеш в hue (0-360)
  const goldenRatio = 137.508; // Примерно угол золотого сечения в градусах
  const hue = Math.abs(hash * goldenRatio) % 360;

  // Saturation: 50–70 для пастельных оттенков
  const saturation = 55 + (Math.abs(hash >> 1) % 10);
  // Lightness: 70–90 для пастельных оттенков
  const lightness = 70 + (Math.abs(hash >> 2) % 10);

  // 3. Генерируем пастельный HSL-цвет
...
export function getColors(palettes, weight = '500') {
  if (typeof palettes === 'string' && colors[palettes.toLowerCase()]?.[weight]) {
    return colors[palettes.toLowerCase()][weight];
  }
