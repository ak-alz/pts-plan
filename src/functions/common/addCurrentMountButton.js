...
export default function addCurrentMountButton(bitrixSessionId) {
  ...
  kanbanItem.forEach((item) => {
    ...
    mountTag.style = `background-image: url(https://files.softicons.com/download/application-icons/toolbar-icons-by-gentleface/png/128/tag.png);
                      width: 15px;
                      height: 15px;
                      right: 5px;
                      top: 23px;
    ...
    mountTag.addEventListener('click', (e) => {
      ...
      xhr.send(`sessid=${bitrixSessionId}&EMITTER=&ACTION[0][OPERATION]=task.update&ACTION[0][ARGUMENTS][id]=${taskId[1]}&ACTION[0][ARGUMENTS][data][TAGS]=${date}`);
    });
