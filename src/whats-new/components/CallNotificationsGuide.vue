...
function handleStorageChanged(changes, area) {
  ...
  if (changes.options) featureEnabled.value = !!changes.options.newValue?.callNotifications;

  // Статусы разовых встреч (пропущена/принята/отклонена) выставляет контент-скрипт в открытой
  // вкладке Bitrix. Без этой синхронизации страница осталась бы со старым списком и затёрла бы
  // их своей копией при следующем сохранении — persistMeetings пишет массив целиком
  if (changes[MEETINGS_STORAGE_KEY]) meetings.value = changes[MEETINGS_STORAGE_KEY].newValue ?? [];

  if (changes[SETTINGS_STORAGE_KEY]) {
    // Настройки эта страница и пишет сама, поэтому сюда прилетает в том числе эхо своей же записи:
    // подменяем ref только на фактическом отличии, иначе watch ниже сохранил бы его снова — и по кругу
    const nextSettings = {...DEFAULT_SETTINGS, ...changes[SETTINGS_STORAGE_KEY].newValue};
    if (JSON.stringify(nextSettings) !== JSON.stringify(toRaw(settings.value))) settings.value = nextSettings;
  }
}
...
async function onImported({skipped = 0} = {}) {
  await loadState();
  toast.add({
    severity: skipped ? 'warn' : 'success',
    summary: 'Импортировано',
    detail: skipped ? `Пропущено записей: ${skipped} — в них не хватает типа, даты или времени.` : undefined,
    life: skipped ? 6000 : 3000,
  });
}
